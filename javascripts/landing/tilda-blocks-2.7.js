...
function t396_doResize(recid) {
    ...
    if (!isOnlyScalable) {
        ...
        rec.find('.tn-atom').each(function() {
            if ($(this).closest('.tn-atom__scale-wrapper').length) {
                $(this).unwrap('.tn-atom__scale-wrapper')
            }
        })
    }
    var ab = rec.find('.t396__artboard');
...
function t396_artboard_build(data, recid) {
    ...
    if (overflowValue === 'visible' || overflowValue === 'visibleX') {
