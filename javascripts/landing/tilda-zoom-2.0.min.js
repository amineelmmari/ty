function t_initZoom() {
    var zoomerWrapper;
    if (!(zoomerWrapper = document.querySelectorAll(".t-zoomer__wrapper")).length) {
...
function t_zoom__closeCarousel() {
    ...
    Array.prototype.forEach.call(zoomerCloseAndBg, (function(el) {
        el.addEventListener("click", (function() {
            var isPopupShown;
            t_zoom_close(), !!document.querySelectorAll(".t-popup_show").length && document.addEventListener("keydown", (function(e) {
                27 === e.keyCode && window.t_store_closePopup && t_store_closePopup(!1)
            }))
        }))
    }))
}
...
function t_zoom_initSwipe() {
    ...
    if (slideItems.length > 1) {
        var hammer = new Hammer(modal, {
            ...
            isScaled = !1;
        window.tzoomopenonce || (hammer.on("panstart", (function() {
            var sliderTrack = document.querySelector(".t-carousel__zoomer__track"),
                sliderTrackTransition;
            "y" !== sliderTrack.getAttribute("data-on-transition") ? sliderTrack && (sliderTrackPosLeft = sliderTrack.getBoundingClientRect().left, sliderTrack.style.transition = "none") : sliderTrackPosLeft = null, isScaled = t_zoom__isScaled(modal)
        })), hammer.on("panmove", (function(event) {
            var sliderTrack = document.querySelector(".t-carousel__zoomer__track"),
                sliderTrackTransition = sliderTrack.getAttribute("data-on-transition"),
                modalOnDrag = modal.getAttribute("data-on-drag"),
                deltaX;
            if ("y" !== sliderTrackTransition && "y" !== modalOnDrag && 1 === event.maxPointers && !isScaled && (Math.abs(event.deltaX) > 40 && sliderTrack.setAttribute("data-on-drag", "y"), sliderTrackPosLeft)) {
            ...
            var sliderTrackTransition = sliderTrack.getAttribute("data-on-transition"),
                modalOnDrag = modal.getAttribute("data-on-drag");
            if ("y" !== sliderTrackTransition && "y" !== modalOnDrag && 1 === event.maxPointers && !isScaled) {
    ...
    document.onkeydown = function(e) {
        switch (e.keyCode) {
            ...
            case 39:
                ...
                break;
            case 27:
                t_zoom_close(), isPopupShown && (document.onkeydown = function(e) {
...
function t_zoom__getEventOnBtn() {
    ...
    Array.prototype.forEach.call(navBtns, (function(navBtn) {
        var arrowBtn;
        document.querySelector(".t-carousel__zoomer__control_" + navBtn.name).addEventListener("click", (function() {
...
function t_zoom__setEventOnBtn(btnMove) {
    var sliderTrack = document.querySelector(".t-carousel__zoomer__track"),
        ...
        modalOnDrag = modal.getAttribute("data-on-drag");
    "y" !== sliderOnTransition && "y" !== modalOnDrag && (t_zoom_unscale(), setTimeout((function() {
...
function t_zoom__addingImgsIntoCarousel(targetElement) {
    var parentBlock = targetElement.closest(".r"),
        images = parentBlock.querySelectorAll(".t-zoomable:not(.t-slds__thumbs_gallery):not(.tn-atom__slds-img)");
    if (parentBlock.querySelectorAll(".t-slds").length) {
        var slider = targetElement.closest(".t-slds");
        slider && (images = slider.querySelectorAll(".t-zoomable:not(.t-slds__thumbs_gallery)"))
    }
    var sliderTrack = document.querySelector(".t-carousel__zoomer__track");
    ...
    if (modal && slideItems.length) {
        ...
        Array.prototype.forEach.call(slideItems, (function(slideItem) {
            ...
            Array.prototype.forEach.call(zoomerComments, (function(comment) {
                ...
                maxCommentsHeight = maxCommentsHeight > commentHeight ? maxCommentsHeight : commentHeight, comment.style.height = maxCommentsHeight + "px"
            }));
            ...
            Array.prototype.forEach.call(zoomedImages, (function(zoomedImg) {
                ...
                ZoomedImgSrc && -1 !== ZoomedImgSrc.indexOf(".svg") && (zoomedImg.style.width = window.innerWidth + "px")
            }));
        ...
        carouselItem && (carouselItem.classList.add("active"), sliderTrack.style.transition = "none", sliderTrack.style.transform = "translateX(" + -carouselPosLeft + "px)", setTimeout((function() {
            sliderTrack.style.transition = ""
        })))
    }
}
...
function t_zoom_showSlide(direction) {
    var sliderTrack = document.querySelector(".t-carousel__zoomer__track"),
        ...
        currentSlideIndex = 0;
    switch (Array.prototype.forEach.call(slideItems, (function(slideItem, index) {
        slideItem === targetItem && (currentSlideIndex = index)
    })), direction) {
        case "next":
            currentSlideIndex = (currentSlideIndex + 1) % slideItems.length, sliderTrack.setAttribute("data-on-transition", "y");
            break;
        case "prev":
            currentSlideIndex = (slideItems.length + (currentSlideIndex - 1)) % slideItems.length, sliderTrack.setAttribute("data-on-transition", "y")
    }
...
function t_zoom_transitForLoop(side) {
    ...
    if (!side) return 1;
    "start" === side && (currentSlideIndex = slideItems.length - 2), "end" === side && (currentSlideIndex = 1), slideOffset = slideItems[currentSlideIndex].offsetLeft, Array.prototype.forEach.call(slideItems, (function(slideItem, index) {
        index === currentSlideIndex ? slideItem.classList.add("active") : slideItem.classList.remove("active")
    })), sliderTrack.style.transition = "none", sliderTrack.style.transform = "translateX(" + -slideOffset + "px)", setTimeout((function() {
        sliderTrack.style.transition = ""
    }))
}
...
function t_zoom_loopSlider() {
    ...
    firstSlideCopy.classList.remove("active"), lastSlideCopy.classList.remove("active"), sliderTrack.insertBefore(lastSlideCopy, sliderTrack.firstChild), sliderTrack.appendChild(firstSlideCopy);
    var slidesCount = (sliderItems = sliderTrack.querySelectorAll(".t-carousel__zoomer__item")).length,
    ...
    Array.prototype.forEach.call(events, (function(event) {
        sliderTrack.addEventListener(event, (function() {
            ...
            })), 0 === currentSlideIndex && t_zoom_transitForLoop("start"), currentSlideIndex === slidesCount - 1 && t_zoom_transitForLoop("end"), sliderTrack.setAttribute("data-on-transition", "")
        }))
    }))
}
...
function t_zoom_initCloseSwipe() {
    var modal = document.querySelector(".t-zoomer__wrapper"),
        ...
        isScaled = !1,
        modalPosition, hammer = new Hammer(modal, {
            domEvents: !0,
            inputClass: Hammer.TouchInput,
            cssProps: {
                touchCollout: "default"
    ...
    hammer.on("panstart", (function() {
        modalPosition = modal.offsetTop, modal.style.position = "none", isScaled = t_zoom__isScaled(modal)
    })), hammer.on("panmove", (function(event) {
        var deltaY = Math.abs(event.deltaY);
        if (("y" !== sliderTrack.getAttribute("data-on-drag") || "y" === modal.getAttribute("data-on-drag")) && (event.angle > -120 && event.angle < -60 || event.angle < 120 && event.angle > 60) && 1 === event.maxPointers && !isScaled) {
...
function t_zoom_closeSwipeHandler(event) {
    ...
    modal.style.transition = "transform 300ms ease-out", Math.abs(event.deltaY) < 40 && (modal.style.transform = ""), "y" !== modal.getAttribute("data-on-drag") || 1 !== event.maxPointers || isScaled || (event.deltaY < -200 || event.velocityY < -.3 ? (modal.style.transform = "translateY(-100vh)", setTimeout((function() {
        t_zoom_close(), modal.style.transform = ""
    }), 300)) : event.deltaY > 200 || event.velocityY > .3 ? (modal.style.transform = "translateY(100vh)", setTimeout((function() {
        t_zoom_close(), modal.style.transform = ""
    }), 300)) : modal.style.transform = ""), modal.setAttribute("data-on-drag", "")
}
...
function t_zoom_checkForScale() {
    ...
    if (zoomedImage && (zoomedImage.onload = function() {
            if (!eventAdded) return windowHeight < zoomedImage.naturalHeight || windowWidth < zoomedImage.naturalWidth ? (zoomedImage.classList.add("loaded"), window.isDoubletapScaleAdded || t_zoom_doubletapScaleInit(), void t_zoom_scale_init()) : void 0
        }, zoomedImage.complete && !eventAdded)) {
        if (eventAdded = !0, windowHeight < zoomedImage.naturalHeight || windowWidth < zoomedImage.naturalWidth) return window.isDoubletapScaleAdded || t_zoom_doubletapScaleInit(), void t_zoom_scale_init();
...
function t_zoom_scale_init() {
    ...
    zoomerScale.style.display = "block", "y" !== zoomerScale.getAttribute("data-zoom-scale-init") && (zoomerScale.setAttribute("data-zoom-scale-init", "y"), zoomerWrapper.addEventListener("click", (function(e) {
        for (var zoomedImage = document.querySelector(".t-carousel__zoomer__item.active .t-carousel__zoomer__img"), zoomerTrack = document.querySelector(".t-carousel__zoomer__track"), zoomerInner = document.querySelector(".t-carousel__zoomer__inner"), target = e.target; target && target != this; target = target.parentNode)
            if (target === zoomerScale) {
                zoomerTrack.setAttribute("data-on-transition", ""), zoomerTrack.style.transition = "none", zoomerTrack.style.transform = "none", zoomedImage.style.maxHeight = "", zoomerWrapper.classList.contains("scale-active") ? t_zoom_unscale() : (zoomerWrapper.classList.add("scale-active"), zoomerInner.classList.add("scale-active"), window.isMobile ? t_zoom_mobileZoomPositioningInit(zoomedImage) : t_zoom_desktopZoomPositioningInit(zoomedImage, e));
                break
            }
    }), !1))
}
...
function t_zoom_desktopZoomPositioningInit(zoomedImage, event) {
    ...
    function imageMoveWidth(e, img) {
        clientXpercent = 100 * (void 0 !== e.touches ? e.touches[0].clientX : e.clientX) / window.innerWidth, imageXpx = -clientXpercent * (img.offsetWidth - window.innerWidth) / 100, img.style.left = imageXpx + "px"
    }

    function imageMoveHeight(e, img) {
        clientYpercent = 100 * (void 0 !== e.touches ? e.touches[0].clientY : e.clientY) / window.innerHeight, imageYpx = -clientYpercent * (img.offsetHeight - window.innerHeight) / 100, img.style.top = imageYpx + "px"
    }
    zoomedImage.style.left = leftCoord + "px", zoomedImage.style.top = topCoord + "px", window.innerWidth < zoomedImage.naturalWidth && window.innerHeight < zoomedImage.naturalHeight ? (clientXpercent = 100 * event.clientX / window.innerWidth, imageXpx = -clientXpercent * (zoomedImage.offsetWidth - window.innerWidth) / 100, clientYpercent = 100 * event.clientY / window.innerHeight, imageYpx = -clientYpercent * (zoomedImage.offsetHeight - window.innerHeight) / 100, zoomedImage.style.left = imageXpx + "px", zoomedImage.style.top = imageYpx + "px", window.isMobile ? zoomedImage.ontouchmove = function(e) {
        imageMoveWidth(e, zoomedImage), imageMoveHeight(e, zoomedImage)
    } : zoomedImage.onmousemove = function(e) {
        imageMoveWidth(e, zoomedImage), imageMoveHeight(e, zoomedImage)
    }) : window.innerWidth < zoomedImage.naturalWidth ? (clientXpercent = 100 * event.clientX / window.innerWidth, imageXpx = -clientXpercent * (zoomedImage.offsetWidth - window.innerWidth) / 100, zoomedImage.style.left = imageXpx + "px", window.isMobile ? zoomedImage.ontouchmove = function(e) {
        imageMoveWidth(e, zoomedImage)
    } : zoomedImage.onmousemove = function(e) {
        imageMoveWidth(e, zoomedImage)
    }) : window.innerHeight < zoomedImage.naturalHeight && (clientYpercent = 100 * event.clientY / window.innerHeight, imageYpx = -clientYpercent * (zoomedImage.offsetHeight - window.innerHeight) / 100, zoomedImage.style.top = imageYpx + "px", window.isMobile ? zoomedImage.ontouchmove = function(e) {
...
function t_zoom_mobileZoomPositioningInit(zoomedImage) {
    ...
    zoomedImage.style.left = leftCoordinate + "px", zoomedImage.style.top = topCoordinate + "px";
    var currentPosition = {
            x: 0,
            y: 0
        },
    ...
    zoomedImage.ontouchstart = function(e) {
        ...
        currentTranslate.x = currentPosition.x + imageOffsetX, currentTranslate.y = currentPosition.y + imageOffsetY, currentTranslate.x > -leftCoordinate && (currentTranslate.x = -leftCoordinate), currentTranslate.x < leftCoordinate && (currentTranslate.x = leftCoordinate), currentTranslate.y > -topCoordinate && (currentTranslate.y = -topCoordinate), currentTranslate.y < topCoordinate && (currentTranslate.y = topCoordinate), zoomedImage.style.transform = "translate(" + currentTranslate.x + "px, " + currentTranslate.y + "px)"
...
function t_zoom_unscale() {
    ...
    if (zoomedItem) {
        ...
        zoomerComments && modal && Array.prototype.forEach.call(zoomedImages, (function(zoomedImage) {
            var imageBordersWidth = modal.offsetHeight - zoomedItem.offsetHeight,
                height = zoomerComments.offsetHeight;
            zoomedImage.onmousemove = null, zoomedImage.ontouchmove = null, zoomedImage.style.transform = "", zoomedImage.style.left = "", zoomedImage.style.top = "", zoomedImage.style.maxHeight = "calc(100vh - " + (imageBordersWidth + height) + "px)"
        }))
    }
...
function t_zoom_lockScroll() {
    ...
    if ((window.isiOS && !window.MSStream || isAndroid) && (window.isiOSVersion && window.isiOSVersion || isAndroid) && (11 === window.isiOSVersion[0] || isAndroid) && !document.body.classList.contains("t-body_scroll-locked")) {
...
function t_zoom_onFuncLoad(funcName, okFunc, time) {
    "function" == typeof window[funcName] ? okFunc() : setTimeout((function checkFuncExist() {
...
function t_zoom_setModalColor(parent) {
    ...
    "#000000" === modalBGColor ? (modalTextColor = "#ffffff", controlsBGColor = "rgba(1, 1, 1, 0.3)", Array.prototype.forEach.call(arrowsWrapper, (function(arrow) {
        arrow.classList.add("t-carousel__zoomer__arrow__wrapper_dark")
    }))) : (modalTextColor = "#000000", controlsBGColor = "rgba(255, 255, 255, 0.3)", Array.prototype.forEach.call(arrowsWrapper, (function(arrow) {
    ...
    })), modalContainer.style.backgroundColor = modalBGColor, modalContainer.style.color = modalTextColor, Array.prototype.forEach.call(modalSvg, (function(svg) {
        "none" === svg.getAttribute("fill") ? svg.setAttribute("fill", "none") : svg.setAttribute("fill", modalTextColor);
        var modalPath = svg.querySelectorAll("path");
        modalPath.length > 0 && Array.prototype.forEach.call(modalPath, (function(path) {
            path.getAttribute("stroke") && path.setAttribute("stroke", modalTextColor), path.getAttribute("fill") && path.setAttribute("fill", modalTextColor)
        }))
...
function t_zoom__isScaled(modal) {
    return window.visualViewport && 1 !== window.visualViewport.scale || modal.classList.contains("scale-active")
}
"loading" !== document.readyState ? window.tzoominited || t_zoom_onFuncLoad("t_initZoom", t_initZoom) : document.addEventListener("DOMContentLoaded", (function() {
